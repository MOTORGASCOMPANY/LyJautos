...
class RevisionExpedientes extends Component
{
        ...
        if ($this->readyToLoad) {
            $expedientes = DB::table('expedientes')
                ...
                ->select(
                    ...
                    'users.name',
                    'taller.nombre',
                    'taller.id as tallerid',
                    DB::raw('(SELECT COUNT(*) FROM imagenes WHERE imagenes.Expediente_idExpediente = expedientes.id AND extension IN ("jpg", "jpeg", "png", "gif", "tif", "tiff", "bmp")) as fotos_count')
                )                
    ...
    public function render()
    {
        $expedientes = Expediente::with([
            'taller',
            'inspector',
            //'certificacion.modificacionDetalle',
            'certificacion',
            'servicio.tipoServicio',            
            'servicio.taller',
            'Archivos'
        ])
        ->where('usuario_idusuario', '!=', Auth::id())
        ...
        if ($this->es != null) {
            $expedientes->where('estado', $this->es);
        }
        
        if ($this->ins != null) {
            $expedientes->where('usuario_idusuario', $this->ins);
        }
        
        if ($this->ta != null) {
            $expedientes->whereHas('servicio', function ($q) {
                $q->where('taller_idtaller', $this->ta);
            });
        }
        
        if ($this->tipoSer != null) {
            $expedientes->whereHas('servicio.tipoServicio', function ($q) {
                $q->where('descripcion', $this->tipoSer);
            });
        }
        
        $expedientes = $expedientes->orderBy($this->sort, $this->direction)
            ->paginate($this->cant);
