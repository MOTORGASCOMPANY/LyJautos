...
class ReporteDocumentosTaller extends Component
{
    ...
    public function documentosSubidos()
    {
        ...
        foreach ($talleres as $taller) {
            foreach ($tiposDocumentos as $tipoDocumento) {
                // Consultamos los documentos por taller y tipo de documento
                $documentos = DocumentoTaller::where('idTaller', $taller->id)
                    ->whereHas('documento', function ($query) use ($tipoDocumento) {
                        $query->where('tipoDocumento', $tipoDocumento->id);
                    })
                    ->with(['documento:id,tipoDocumento,fechaInicio,fechaExpiracion']) // Optimización de consulta
                    ->get();

                // Agregamos los documentos a la lista con sus fechas
                foreach ($documentos as $documento) {
                    $fechaExpiracion = $documento->documento->fechaExpiracion;

                    // Determinar estado del documento
                    $estado = ($fechaExpiracion && $hoy->greaterThan($fechaExpiracion))
                        ? 'Caduco'
                        : 'Vigente';
                    ...
                    $documentosSubidos->push([
                        'taller'         => $taller->nombre,
                        'tipoDocumento'  => $tipoDocumento->nombreTipo,
                        'combustible'    => $documento->combustible,
                        'fechaInicio'    => $documento->documento->fechaInicio,
                        'fechaExpiracion' => optional($documento->documento->fechaExpiracion)->format('d/m/Y') ?? 'N/A',
                        'estado'          => $estado,
                    ]);
                }
            }
