...
class ReportesMtg extends Component
{
    ...
    public function procesar()
    {
        ...
        // Filtrar los registros de tabla donde estado sea 1 o 3 para que no considere los que están en estado 2 (anulados)
        $tablaFiltrada = collect($this->tabla)->filter(function ($item) {
            // Verificamos si el tipo_modelo es Certificacion y su estado NO debe ser 2
            if ($item['tipo_modelo'] === 'App\Models\Certificacion') {
                return $item['estado'] !== 2;
            }

            // Para los otros modelos, no filtramos por estado
            return true;
        })->values()->all(); // Asegura que sea un array numerado
    ...
    public function generateChartData()
    {
        ...
        $mdata = $datosCombinados;

        $data = $mdata->groupBy('servicio')->map(function ($group) {
            return $group->count();
        });
    ...
    public function exportarExcel()
    {
        ...
        if ($data->isNotEmpty()) {
            ...
            return Excel::download(new ReporteCalcularExport($data), 'Reporte de ' . $this->fechaInicio . ' al ' . $this->fechaFin .'.xlsx');
        }
    }
    ...
    public function generaData()
    {
        ...
        if ($this->user->hasRole('inspector')) {
            ...
            $desmontes = Desmontes::idTalleres($this->taller)
                ...
                ->rangoFecha($this->fechaInicio, $this->fechaFin)
                ->get();
            //TODO CERT TEMPORALES PARA INSPECTOR:
            ...
            //TODO CERT TEMPORALES PARA OFICINA:
            $cerTemp = CertificacionTemporal::idTalleres($this->taller)
                ->IdInspectores($this->ins)
                ->IdTipoServicios($this->servicio)
                ->rangoFecha($this->fechaInicio, $this->fechaFin)
        ...
        foreach ($certificaciones as $certi) {
            ...
            //$precio = number_format($certi->estado == 2 ? 0 : $certi->precio, 2, '.', '');
