...
class BoletasArchivos extends Component
{
    ...
    public function mount()
    {
        $this->cargarBoletaConArchivos();
    }

    private function cargarBoletaConArchivos()
    {
        $this->boleta = Boleta::with('boletaarchivo')->find($this->idBoleta);

        foreach ($this->boleta->boletaarchivo as $doc) {
            if ($doc->migrado == 1) {
