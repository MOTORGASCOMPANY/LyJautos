...
class RevisionInventario extends Component
{
    ...
    public function consultar()
    {
        ...
        /*$this->resultado = Material::where("idUsuario", $this->inspector)
            ...
            ->get();*/
        $this->resultado = Material::with('certificacion.Vehiculo') // Agregamos la relación para obtener la placa
    ...
    public function resumen()
    {
        ...
        $usuarios = User::role('inspector')
            ->whereIn('id', $materiales->keys()->all())
            ->orderBy('name')
            ->get()
            ->keyBy('id');

        // Filtrar materiales para incluir solo los de usuarios con el rol de inspector
        $materiales = $materiales->filter(function ($_, $idUsuario) use ($usuarios) {
            return $usuarios->has($idUsuario);
        });

        // Obtener el conteo de materiales anulados agrupados por usuario
        $anulados = Material::where('estado', 5)
            ->whereNull('devuelto')
            ->whereIn('idUsuario', $usuarios->keys()->all()) // Asegurar que sean solo inspectores
            ->groupBy('idUsuario')
