...
class Certificacion extends Model
{
    ...
    // relacion con modificacion para distinguir una modi normal de una modi de motor
    ...
    public static function certificarGlp(Taller $taller, Taller $tallerAuto, Servicio $servicio, Material $hoja, vehiculo $vehiculo, User $inspector, $externoValue)
    {
        ...
        $precio = $esContado 
            ...
            : ($externoValue == 0 
                ? $servicio->precio 
                : optional(PrecioInspector::where([
    ...
    public static function certificarGnv(Taller $taller, Servicio $servicio, Material $hoja, vehiculo $vehiculo, User $inspector, $externoValue, $placaantigua)
    {
    ...
    public static function certificarModi(Taller $taller, Servicio $servicio, Material $hoja, vehiculo $vehiculo, User $inspector, $externoValue, $esModiMotor = false)
    {
        // Paso 1: Buscar idSalida en SalidaDetalle con el idMaterial
        $idSalida = optional(SalidaDetalle::where('idMaterial', $hoja->id)->first())->idSalida;
        // Paso 2: Validar si idSalida existe en Contado
    ...
    public static function tramiteChipDeterioro($taller, $servicio, User $inspector, $nombre, $placa, $externoValue)
    {
        if ($externoValue == 0) {
            $precio = Servicio::find($servicio)->precio;
        } elseif ($externoValue == 1) {
    ...
    public static function certificarGnvOverhul(Taller $taller, Servicio $servicio, Material $hoja, vehiculo $vehiculo, User $inspector, Material $chip, $externoValue)
    {
        // Condición para asignar el precio según el valor de $externoValue
        $precio = $externoValue == 0
        ? $servicio->precio
        : PrecioInspector::where([
            ['idServicio', $servicio->TipoServicio->id],
            ['idUsers', $inspector->id]
        ])->value('precio') ?? 0;
