<div>
    ...
    <x-jet-dialog-modal wire:model="addDocument">
        <x-slot name="title">
            <h1 class="text-xl font-bold">Agregar comprobantes</h1>
        </x-slot>
        <x-slot name="content">
            {{-- 
            <div class="mb-4">
            ...
            @hasanyrole('administrador|Administrador del sistema')
                <div class="mb-4">
                    <x-jet-label for="idBoletas" value="Clave de Boletas (separados por comas)" />
                    <x-jet-input id="idBoletas" placeholder="Ejemplo: 1, 2, 3" type="text" wire:model="idBoletas" class="mt-1 block w-full" />
            ...
            <div class="m-4">
                <!-- Checkbox para mostrar el input de monto_pagado -->
                <div class="flex items-center my-4">
                    <input type="checkbox" id="mostrarMonto" wire:model="mostrarMonto"
                        class="mr-2 text-indigo-500 focus:ring-indigo-400 focus:ring-opacity-25 border-gray-300 rounded">
                    <label for="mostrarMonto" class="text-gray-700">¿Es Pago Parcial?</label>
                </div>

                <!-- Input de monto_pagado condicional -->
                @if ($mostrarMonto)
                    <div class="my-4">
                        <x-jet-label for="monto_pagado" value="Monto:" />
                        <x-jet-input id="monto_pagado" type="number" wire:model="monto_pagado" step="0.01" min="0"
                            class="block w-full mt-1" placeholder="Ingrese el monto parcial" />
                        <x-jet-input-error for="monto_pagado" />
                    </div>
                @endif
            </div>
        </x-slot>
