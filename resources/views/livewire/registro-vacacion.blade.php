            ...
            <table class="min-w-full border-collapse overflow-hidden rounded-2xl">
                <thead class="bg-slate-600 text-white">
                    <tr>
                        ...
                        <th class="px-6 py-3 text-left text-sm font-semibold">Días Tomados</th>
                        <th class="px-6 py-3 text-left text-sm font-semibold">Fecha Inicio</th>
                        <th class="px-6 py-3 text-left text-sm font-semibold">Razón</th>
                        <th class="px-6 py-3 text-left text-sm font-semibold">Especial</th>
                        <th class="px-6 py-3 text-left text-sm font-semibold">Observación</th>
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    @foreach ($vacacionesAsignadas as $index => $vac)
                        <tr class="hover:bg-gray-100 transition-colors duration-150">
                            <td class="px-6 py-3 text-sm text-gray-700">{{ $vac->id }}</td>
                            <td class="px-6 py-3 text-sm text-gray-700">
                                {{ $vac->Vacacion->contrato->empleado->name }}
                            </td>
                            <td class="px-6 py-3 text-sm text-gray-700">{{ $vac->tipo }}</td>
                            <td class="px-6 py-3 text-sm text-gray-700">{{ $vac->d_tomados }}</td>
                            <td class="px-6 py-3 text-sm text-gray-700">{{ $vac->f_inicio }}</td>
                            <td class="px-6 py-3 text-sm text-gray-700">{{ $vac->razon }}</td>
                            <td class="px-6 py-3 text-sm text-gray-700">
                                <div class="flex items-center">
                                    <x-jet-checkbox class="h-4 w-4 text-indigo-600 rounded-lg" disabled
                                        :checked="$vac->especial" />
                                </div>
                            </td>
                            <td class="px-6 py-3 text-sm text-gray-700">{{ $vac->observacion }}</td>
                            <td class="px-6 py-3 text-sm text-gray-700">
                                <div class="flex space-x-2">
                                    <a href="{{ route('VacacionAsig', ['id' => $vac->id]) }}" target="_blank"
        ...
        <div class="mt-4">
            @if ($vacacion)
                <div class="grid grid-cols-3 gap-4">
                    <div class="flex items-center justify-between bg-white p-3 rounded-md shadow">
                        <span class="text-blue-600">Días Ganados</span>
                        <span class="text-blue-800">{{ $vacacion->dias_ganados }}</span>
                    </div>
                    <div class="flex items-center justify-between bg-white p-3 rounded-md shadow">
                        <span class="text-green-600">Días Tomados</span>
                        <span class="text-green-800">{{ $vacacion->dias_tomados }}</span>
                    </div>
                    <div class="flex items-center justify-between bg-white p-3 rounded-md shadow">
                        <span class="text-yellow-600">Días Restantes</span>
                        <span class="text-yellow-800">{{ $vacacion->dias_restantes }}</span>
                    </div>
                </div>
            @endif
        </div>
