<div class="min-h-screen bg-gray-100 py-10">
    <div class="max-w-screen-2xl mx-auto px-6">
        ...
        <div class="space-y-4" id='data_1'>
                    ...
                    <table class="table-auto w-full text-sm text-left">
                        <thead class="bg-indigo-500 text-white">
                            <tr>
                                ...
                                @foreach ($semorderTalle as $semana)
                                    <th class="px-4 py-2 font-medium">{{ $semana }}</th>
                                @endforeach
                            </tr>
                        </thead>
                        <tbody>
                            @foreach ($tablaFinalTalleres as $taller => $montos)
                                <tr class="{{ $loop->even ? 'bg-gray-100' : 'bg-white' }}">
                                    <td class="border px-4 py-2 text-gray-700">{{ $taller }}</td>
                                    @foreach ($semorderTalle as $semana)
                                        <td class="border px-4 py-2 text-gray-700">
                                            {{ isset($montos[$semana]) ? number_format($montos[$semana], 2) : '0' }}
                                        </td>
                                    @endforeach
                                </tr>
                            @endforeach
                            <tr class="bg-indigo-100">
                                <td colspan="{{ count($semorderTalle) }}"
                                    class="px-4 py-2 text-right font-bold text-indigo-800">
                                     Suma Total:
                                </td>
                                <td class="px-4 py-2 font-bold text-indigo-800">
                                    S/ {{ number_format($totalTalleres, 2) }}
                                </td>
                            </tr>
                        </tbody>
                    </table>
            ...
            @endif
            <!-- Tabla Inspectores-->
            @if (!empty($tablaFinalInspectores))
                <div class="text-center mb-2">
                    <h4 class="font-extrabold text-gray-800 uppercase tracking-wide">
                    ...
                    <table class="table-auto w-full text-sm text-left">
                        <thead class="bg-indigo-500 text-white">
                            <tr>
                                ...
                                @foreach ($semorderIns as $semanaIns)
                                    <th class="px-4 py-2 font-medium">{{ $semanaIns }}</th>
                                @endforeach
                            </tr>
                        </thead>
                        <tbody>
                            @foreach ($tablaFinalInspectores as $inspector => $montos)
                                <tr class="{{ $loop->even ? 'bg-gray-100' : 'bg-white' }}">
                                    <td class="border px-4 py-2 text-gray-700">{{ $inspector }}</td>
                                    @foreach ($semorderIns as $semana)
                                        <td class="border px-4 py-2 text-gray-700">
                                            {{ isset($montos[$semana]) ? number_format($montos[$semana], 2) : '0' }}
                                        </td>
                                    @endforeach
                            ...
                            @endforeach
                            <tr class="bg-indigo-100">
                                <td colspan="{{ count($semorderIns) }}"
                                    class="px-4 py-2 text-right font-bold text-indigo-800">
                                    Suma Total:
                                </td>
                                <td class="px-4 py-2 font-bold text-indigo-800">
                                    S/ {{ number_format($totalInspectores, 2) }}
                                </td>
                            </tr>
                        </tbody>
                    </table>
                ...
                <div class="overflow-x-auto bg-white shadow-md rounded-lg">
                    <table class="table-auto w-full text-sm text-left">
                        ...
                        <tbody>
                            ...
                            @endforeach
                            <tr class="bg-indigo-100">
                                <td class="px-4 py-2 text-right font-bold text-indigo-800">Suma Total:</td>
                                <td class="px-4 py-2 font-bold text-indigo-800">
                                   S/ {{ number_format($totalContados, 2) }}
                                </td>
                            </tr>
                        </tbody>
                    </table>
                    ...
                    <table class="min-w-full bg-white">
                        <thead>
                            <tr class="bg-indigo-200 text-indigo-800">
                                ...
                                <th class="px-4 py-2 text-right font-bold text-indigo-800">
                                    S/ {{ number_format($cierre, 2) }}</th>
                            </tr>
                        </thead>
                    </table>
                </div>
        ...
        <div class="overflow-x-auto bg-white shadow-md rounded-lg">
            <table class="table-auto w-full text-sm text-left">
                <thead class="bg-indigo-500 text-white">
                    <tr>
                        <th class="px-4 py-2 font-medium">Talleres</th>
                        @foreach (array_keys(collect($tablaFinalTalleres)->collapse()->toArray()) as $semana)
                            <th class="px-4 py-2 font-medium">{{ $semana }}</th>
                        @endforeach
                    </tr>
                </thead>
                <tbody>
                        ...
                        @endforeach
                        <tr class="bg-indigo-100">
                            <td colspan="{{ collect($tablaFinalTalleres)->collapse()->count() }}"
                                class="px-4 py-2 text-right font-bold text-indigo-800">
                                Total:
                            </td>
                            <td class="px-4 py-2 font-bold text-indigo-800">
                                S/ {{ number_format($sumaTotalTalleres, 2) }}
                            </td>
                        </tr>
                </tbody>
            </table>
        ...
        <div class="overflow-x-auto bg-white shadow-md rounded-lg">
            <table class="table-auto w-full text-sm text-left">
                <thead class="bg-indigo-500 text-white">
                    <tr>
                        <th class="px-4 py-2 font-medium">Inspector</th>
                        @foreach (array_keys(collect($tablaFinalInspectores)->collapse()->toArray()) as $semana)
                            <th class="px-4 py-2 font-medium">{{ $semana }}</th>
                        @endforeach
                    </tr>
                </thead>
                ...
                <tr class="bg-indigo-100">
                    ...
                    <td class="px-4 py-2 font-bold text-indigo-800"
                        S/ {{ number_format($sumaTotalInspectores, 2) }}
                    </td>
                </tr>
            </table>
        </div>
