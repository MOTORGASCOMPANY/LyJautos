            ...
            <div class=" items-center md:block sm:block">
                <div class="p-2 w-64 my-4 md:w-full">
                    <h2 class="text-indigo-600 font-bold text-3xl">
                        ...
                        &nbsp;ACTUALIZAR PRECIO DE CERTIFICACIONES
                    </h2>
                </div>
        ...
        @if (isset($reportePorInspector))
            <div>
                <div class="p-2 flex justify-end m-auto flex items-space-around">
                    <a wire:click="ver({{ json_encode(collect($reportePorInspector)->pluck('id')->toArray()) }}, {{ json_encode(collect($reportePorInspector)->pluck('servicio')->unique()->toArray()) }})"
                ...
                @foreach ($reportePorInspector->groupBy('inspector') as $inspector => $reporte)
                    <div class="bg-gray-200 px-8 py-2 rounded-xl w-full">
                        <div class="p-2 w-full justify-between m-auto flex items-space-around">
                            <h2 class="text-indigo-600 text-xl font-bold">{{ $inspector }}</h2>
                        </div>
                                ...
                                <div class="overflow-hidden">
                                        ...
                                        <tbody>
                                            @foreach ($reporte as $key => $item)
                                                <tr class="bg-orange-200 hover:bg-orange-300">
                                                    ...
                                                    <td class="whitespace-nowrap border-r px-6 py-3">
                                                        <input type="checkbox" wire:model="selectedCertificaciones"
                                                            value="{{ $item['id'] }}">
                                                    </td>
                                                </tr>
                                            @endforeach
                                            <tr class="bg-green-200">
                                                <td colspan="11" class="border-r px-6 py-3 font-bold text-right">
                                                    Total:
                                                </td>
                                    ...
                                    <div class="mt-4">
                                        <ul class="grid grid-cols-2 gap-4">
                                            @foreach (collect($reporte)->groupBy('servicio') as $tipoServicio => $detalle)
