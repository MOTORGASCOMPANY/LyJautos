    ...
    <x-jet-dialog-modal wire:model="editando" wire:loading.attr="disabled" wire:target="deleteFile">
        ...
        <x-slot name="content">
            @if ($expediente)
                ...
                <!-- mostrar observaciones si el estado es observado -->
            ...
            <div class="mb-4">
                <div class="flex items-center">
                    ...
                    <p class="ml-2 text-xs text-gray-500 italic">
                        <strong>MotorIA</strong> puede cometer errores, verifica los resultados.
                    </p>
                </div>
                {{--<x-jet-label value="Fotos:" />--}}
                ...
                <section class="mt-4 overflow-hidden border-dotted border-2 text-gray-700 "
                    ...
                    <div class="container px-5 py-2 mx-auto lg:px-32">
                        <div class="flex flex-wrap -m-1 md:-m-2">
                            ...
                            @foreach ($files as $fil)
                                <div class="flex flex-wrap w-1/3 ">
                                    <div class="w-full p-1 items-center justify-center">
                                        ...
                                        @else
                                            <img alt="gallery"
                                                class="mx-auto flex object-cover object-center w-36 h-36 rounded-lg" src="{{ $fil->url_temporal }}">
                                        @endif
                                        --}}
                                        <img alt="gallery"
                                            class="mx-auto flex object-cover object-center w-36 h-36 rounded-lg"
                                            src="{{ $fil->url }}">
                                        <select wire:model="fileTipos.{{ $fil->id }}" class="mt-2 w-full border-gray-300 rounded-md text-sm">
                            ...
                            <!-- Fotos nuevas (temporal) -->
                            {{--
                                @if (count($fotosnuevas))
                                    @foreach ($fotosnuevas as $key => $otro)
                                        <div class="flex flex-wrap w-1/3 " wire:key="foto-nueva-{{ $key }}-{{ $otro->getFilename() }}">
                                            <div class="w-full p-1 items-center justify-center">
                                                <img alt="gallery"
                                                ...
                                                <select wire:model="tiposFotosNuevas.{{ $key }}" class="mt-2 w-full border-gray-300 rounded-md text-sm">
                                                    <option value="">-- Seleccione tipo --</option>
                                                    @foreach ($tipos_imagen as $tipo)
                                                        <option value="{{ $tipo->id }}">{{ $tipo->descripcion }}</option>
                                                    @endforeach
                                ...
                                @endif
                            --}}
                            @foreach ($fotosnuevas as $id => $file)
                                <div class="flex flex-wrap w-1/3" wire:key="foto-nueva-{{ $id }}">
                                    <div class="w-full p-1 items-center justify-center">
                                        <img class="mx-auto object-cover object-center w-36 h-36 rounded-lg shadow-lg" src="{{ $file->temporaryUrl() }}">

                                        <select wire:model="tiposFotosNuevas.{{ $id }}" class="mt-2 w-full border-gray-300 rounded-md text-sm">
                                            <option value="">-- Seleccione tipo --</option>
                                            @foreach ($tipos_imagen as $tipo)
                                                <option value="{{ $tipo->id }}">{{ $tipo->descripcion }}</option>
                                            @endforeach
                                        </select>
                                         <!-- confianza de la IA si existe %-->
                                        @if(!empty($confidences[$id]))                                        
                                            <p class="text-xs text-gray-500 mt-1 text-center">
                                                <strong>MotorIA</strong> confianza: {{ number_format($confidences[$id], 2) }}%
                                            </p>
                                        @endif
                                        <a class="flex" wire:click="deleteFileUpload('{{ $id }}')">
                                            <i class="fas fa-trash mt-1 mx-auto hover:text-indigo-400"></i>
                                        </a>
                                    </div>
                                </div>
        ...
        <x-slot name="footer">
            ...
            <x-jet-button wire:click="actualizar" wire:loading.attr="disabled"
                wire:target="actualizar,documentosnuevos,fotosnuevas">
                Actualizar
            </x-jet-button>
        </x-slot>
    </x-jet-dialog-modal>
